...
// Assuming a Student struct or class is defined elsewhere, or needs to be defined here.
// For the purpose of providing the complete program, I'll define a basic one.
struct Student {
    std::string name;
    int age;
    std::string grade; // Changed to string to match input expectations (e.g., "A", "B+", "C")
};
...
void addStudent() {
    ...
    std::cin.ignore(); // To consume the leftover newline character from previous cin
...
void searchStudentByName() {
    ...
    if (inFile.is_open()) {
        ...
        while (std::getline(inFile, line)) {
            ...
            int age;
            std::string grade;
            iss >> n >> age >> grade; // Extract name, age, grade
